                        ...
                        {!countryCollapsed && country.cities.map(cg2 => (
                          <div key={`${cg2.state}/${cg2.city}`} style={{ marginBottom: 16 }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, paddingLeft: 12 }}>
                              {cg2.trips.map(trip => <TripCard key={trip.id} trip={trip} deleteConfirm={deleteConfirm} setDeleteConfirm={setDeleteConfirm} onSelectTrip={onSelectTrip} onEdit={setEditingTrip} setTrips={setTrips} />)}
                            </div>
                          </div>
                        ))}
                      </div>
            ...
            {ungrouped.length > 0 && (
              <div style={{ marginBottom: 28 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {ungrouped.map(trip => <TripCard key={trip.id} trip={trip} deleteConfirm={deleteConfirm} setDeleteConfirm={setDeleteConfirm} onSelectTrip={onSelectTrip} onEdit={setEditingTrip} setTrips={setTrips} />)}
                </div>
              </div>
            )}
          </div>
